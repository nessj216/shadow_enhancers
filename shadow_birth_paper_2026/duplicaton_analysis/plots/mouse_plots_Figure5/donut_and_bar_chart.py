...
OUTPUT_DIR = PAPER_ROOT / "plots" / "output_pngs" / "mouse"
...
# ====== Input files ======
BREAKDOWN_FILE = str(HERE / "final_BREAKDOWN_from_CollatedComparisons_shadowset_aware_plus_double_flanks_e1e-45.csv")
TOTAL_BED_FILE = str(HERE / "shadowenhancercombined.bed")
csv_file = BREAKDOWN_FILE
OUT_PNG = str(OUTPUT_DIR / "donut_mouse_duplications.png")
...
print(TOTAL)
# Unique gene sets represented in each source group
enh_gene_sets = set(
    df.loc[df["source"].astype(str).str.strip() == "enhancer_hit", "gene_name"]
    .dropna()
    .astype(str)
    .str.strip()
)
...
flank_gene_sets = set(
    df.loc[df["source"].astype(str).str.strip() == "flank", "gene_name"]
    .dropna()
    .astype(str)
    .str.strip()
)

# "Any hit" = union of enhancer-hit and flank-hit gene sets
any_gene_sets = enh_gene_sets | flank_gene_sets
ANY = len(any_gene_sets)
NONE = TOTAL - ANY

# Breakdown of hit sets:
# ENH = all gene sets found in enhancer_hit
# FLANK = flank-only gene sets not already counted in enhancer_hit
ENH = len(enh_gene_sets)
FLANK_ONLY_GENE_SETS = flank_gene_sets - enh_gene_sets
FLANK = len(FLANK_ONLY_GENE_SETS)

print(f"TOTAL gene sets in BED col4: {TOTAL}")
print(f"Enhancer-hit gene sets: {ENH}")
print(f"Flank gene sets (all): {len(flank_gene_sets)}")
print(f"Flank-only gene sets: {FLANK}")
print(f"Any hit gene sets: {ANY}")
print(f"No-hit gene sets: {NONE}")

if ANY != ENH + FLANK:
    print("Warning: ANY != ENH + FLANK. Check for overlap handling.")
    print("This code defines FLANK as flank-only, so ANY should equal ENH + FLANK.")
...
# ---- Save ----
plt.savefig(OUTPUT_DIR / "stacked_bars_gene_binned_percent.png", dpi=600, bbox_inches="tight")
plt.savefig(OUTPUT_DIR / "stacked_bars_gene_binned_percent.pdf", dpi=600, bbox_inches="tight")
plt.show()
...
enh_double_pct = 100 * enh_double / enh_total if enh_total > 0 else 0
enh_single_pct = 100 * enh_single / enh_total if enh_total > 0 else 0
enh_none_pct   = 100 * enh_none   / enh_total if enh_total > 0 else 0

# Flank cohort is entirely double-flank by definition
flank_pct = 100 if flank_total > 0 else 0
...
# Enhancer stacked percentage bar
bottom = 0
ax.bar(x[0], enh_double_pct, bottom=bottom, color=C_DOUBLE, edgecolor="white", linewidth=1.2)
bottom += enh_double_pct

ax.bar(x[0], enh_single_pct, bottom=bottom, color=C_SINGLE, edgecolor="white", linewidth=1.2)
bottom += enh_single_pct

ax.bar(x[0], enh_none_pct, bottom=bottom, color=C_NONE, edgecolor="white", linewidth=1.2)

# Flank cohort percentage bar
ax.bar(x[1], flank_pct, bottom=0, color=C_DOUBLE, edgecolor="white", linewidth=1.2)
...
start = 0
label_segment(x[0], start, enh_double_pct, f"{enh_double_pct:.1f}%")
start += enh_double_pct

label_segment(x[0], start, enh_single_pct, f"{enh_single_pct:.1f}%")
start += enh_single_pct

label_segment(x[0], start, enh_none_pct, f"{enh_none_pct:.1f}%")

label_segment(x[1], 0, flank_pct, f"{flank_pct:.1f}%")
...
# ---- Labels below bars ----
ax.text(x[0], -6, labels[0], ha="center", va="top", fontsize=8)
ax.text(x[1], -6, labels[1], ha="center", va="top", fontsize=8)

ax.set_xlim(-0.6, len(labels) - 0.4)
ax.set_ylim(-14, 102)

plt.savefig(
    OUTPUT_DIR / "stacked_bars_from_csv_percent.png",
    dpi=600,
    bbox_inches="tight"
)
plt.savefig(
    OUTPUT_DIR / "stacked_bars_from_csv_percent.pdf",
    dpi=600,
    bbox_inches="tight"
)
