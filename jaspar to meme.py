def jaspar_to_meme(jaspar_file_path, meme_file_path):
    """
    Convert a JASPAR PWM file to MEME format by calculating column-wise probabilities.
    ...
    for line in jaspar_lines:
        if line.startswith('>'):
            if motif_id:  # Save the previous motif before starting a new one
                meme_content.extend(format_meme_motif(motif_id, motif_name, matrix))
                matrix = []
...
def format_meme_motif(motif_id, motif_name, matrix):
    """
...
meme_file_path = 'output_meme_file.meme'  # Update this with your desired output MEME file path
