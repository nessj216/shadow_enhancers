'''The denominator across the row is the total shadows whether or not they had a +hit duplication enhancer'''
import collections
...
##############################################################################
# 2) Read & bucket hit file
#    Bucket sets based on how many hits they have:
#       - exactly 1          -> "1 hit"
#       - exactly 2          -> "2 hit"
#       - between 3 and 4    -> "3-4 hit"
#       - >= 5               -> "5+ hit"
##############################################################################
hit_file = "/Users/jillianness/Desktop/SEbirthanalysis1024/Duplications_BLAST/HEATMAP_stuff/BLAStHITS_deduplicated_lines.csv"
#te_file ="/Users/jillianness/Desktop/SEbirthanalysis1024/Duplications_BLAST/HEATMAP_stuff/blast_hits_dedup_10-5.csv"
hit_counts = collections.Counter()

with open(hit_file, 'r') as tf:
    for line in tf:
        if line.strip():
            cols = line.strip().split(',')
            # Assume set name is in column 4 (index 3)
            set_name = cols[0]
            hit_counts[set_name] += 1

hit_buckets = {}
for set_name, count in hit_counts.items():
    if count == 1:
        hit_buckets[set_name] = "1 Pair"
    elif count == 2:
        hit_buckets[set_name] = "2 Pairs"
    elif count ==3:
        hit_buckets[set_name] = "3 Pairs"

    else:
        # count >= 5
        hit_buckets[set_name] = "4+ Pairs"

##############################################################################
# 3) Cross‐tabulate into a 3×4 table: (shadow bucket) × (TE bucket)
##############################################################################
shadow_categories = ["2 shadows", "3-4 shadows", ">=5 shadows"]
hits_categories     = ["1 Pair", "2 Pairs", "3 Pairs", "4+ Pairs"]

# combo_counts[(s_cat, t_cat)] = how many sets are in shadow bucket s_cat + TE bucket t_cat
combo_counts = {}
for s_cat in shadow_categories:
    for t_cat in hits_categories:
        combo_counts[(s_cat, t_cat)] = 0

for set_name, s_bucket in shadow_buckets.items():
    if set_name in hit_buckets:
        t_bucket = hit_buckets[set_name]
...
print("\nProportion matrix (rows = shadow categories, cols = hit categories):")
...
for row_idx, s_cat in enumerate(shadow_categories):
    for col_idx, t_cat in enumerate(hits_categories):
...
# Keep the general font size for other elements:
plt.rcParams.update({'font.size': 14})



# If you also want to adjust title size separately, do:
#ax.set_title("Proportion of Sets in Each Shadow Category by TE Category", fontsize=14)
...
ax.set_xticks(np.arange(len(hits_categories)))
ax.set_xticklabels(hits_categories)

# Y-axis: Shadow categories, with row sums under the label
shadow_labels = []
for s_cat in shadow_categories:
    total_in_cat = shadow_cat_counts[s_cat]
    # Format label with the sum on a new line
    shadow_labels.append(f"{s_cat}\nn={total_in_cat}")

ax.set_yticks(np.arange(len(shadow_categories)))
ax.set_yticklabels(shadow_labels)


# Then make specific items bigger:
ax.set_xlabel("Pair Duplication #", fontsize=16)
ax.set_ylabel("Total Shadows in Set", fontsize=16)
# Colorbar
cbar = plt.colorbar(im, ax=ax)
cbar.set_label("Proportion of Sets", rotation=270, labelpad=16,fontsize=16)
...
print("Heatmap fractions (row = Shadow Category, col = hit Category):")
for row_idx, s_cat in enumerate(shadow_categories):
    for col_idx, t_cat in enumerate(hits_categories):
        fraction = heatmap_data[row_idx][col_idx]
        print(f"  {s_cat} | {t_cat} -> {fraction:.3f}")
