...
directory = "/Users/jillianness/Desktop/TOBIAS091922/D5"
path_enh=0
...
# Create a horizontal bar plot using the 'out' dictionary
tf_names = [list(d.keys())[0] for d in out]
tf_avg_values = [list(d.values())[0] for d in out]

plt.barh(tf_names, tf_avg_values, color='skyblue')
plt.xlabel('Average Value')
plt.ylabel('TF')
plt.title('Average Values of Kni+0 TFs')

# Label each bar with its value
for tf_name, avg_value in zip(tf_names, tf_avg_values):
    plt.text(avg_value, tf_name, f'{avg_value:.2f}', ha='center', va='center', fontsize=8, color='black')
